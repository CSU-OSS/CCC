...
5. 将数据集中所有`is_CCS` = 1的数据单独导出为一个文件，并为每一条数据增加`commit_type`与`commit_scope`字段，提取message中的type与scope
...
7. 将`ccs_commits_dataset`的`parquet`文件转为`json`文件（如果需要）
